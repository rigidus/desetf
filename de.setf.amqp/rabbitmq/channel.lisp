...
(defmethod channel-ticket ((channel channel))
  (de.setf.amqp.utility:channel-ticket channel))

;;; define a consumer and a deilvery

(defclass queueingconsumer. ()
  ((queue :initform (make-instance 'de.setf.amqp.utility:queue)
...
(defmethod consumer-next-delivery ((consumer queueingconsumer.))
  (unless (consumer-empty-p consumer)
    (de.setf.amqp.utility:dequeue (consumer-queue consumer))))
...
(defmethod consumer-empty-p ((consumer QueueingConsumer.))
  (error "tmp-error-1")
  ;; (and (de.setf.amqp.implementation::collection-empty-p (consumer-queue consumer))
  ;;      (let ((count 0))
...
(defmethod consumer-arrived-count ((consumer QueueingConsumer.))
  (if (consumer-empty-p consumer)
    0
    (de.setf.amqp.implementation::collection-size (consumer-queue consumer))))
        ...
        #'(lambda (channel class method &rest args)
            (let* ((body (apply #'amqp:respond-to-deliver class args))
                   (message (make-instance 'queueingconsumer$delivery
                              :body body
                              :content-type (de.setf.amqp.utility::class-mime-type class)
                              :envelope (apply #'make-envelope
                                               (de.setf.amqp.utility:method-arguments method))
                              :properties (de.setf.amqp.utility:class-properties class)))
                   (consumer (the-consumer channel)))
              (when consumer
                (de.setf.amqp.utility:enqueue message (consumer-queue consumer)))
              message)))
